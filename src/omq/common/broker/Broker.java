...
import omq.supervisor.broker.SlowRemoteBrokerImpl;
...
public class Broker {
     ...
     *
     * @param reference - Binding name
     * @param contract - Remote Interface
     * @return newProxy
    ...
    @SuppressWarnings("unchecked")
    public synchronized <T extends Remote> T lookup(String reference, String userID, Class<T> contract) throws RemoteException {
        try {
            ...
            if (!proxies.containsKey(reference)) {
                Proxymq proxy = null;
                    ...
                    proxy = new Proxymq(reference, contract, userID, this);
     ...
     *
     * @param reference - Binding name
     * @param remote - RemoteObject to bind
     * @throws RemoteException If the remote operation failed
     ...
     * @param reference - Binding name
     * @param remote - RemoteObject to bind
     * @param env - RemoteObject environment. You can set how many threads will
     * be listen to the reference, the multiqueue name and the properties of the
     * object queue and multiqueue
     * @throws RemoteException If the remote operation failed
     * @throws AlreadyBoundException If name is already bound.
     */
    public void bind(String reference, RemoteObject remote, Properties env) throws RemoteException, AlreadyBoundException {
        ...
        try {
            ...
            if (statisticsMap != null) {
                StatisticsThread stats = new StatisticsThread(remote);
    ...
    public void bind(String reference, RemoteObject remote, String userID, boolean slowQueue) throws RemoteException, AlreadyBoundException, Exception {
        bind(reference, remote, userID, slowQueue, environment);
    }

    /* slowqueue indicates whether the object will consume messages from the slow queue or not.
        DO NOT USE USERID = "broker" as it is used by the brokers
    */
    public void bind(String reference, RemoteObject remote, String userID, boolean slowQueue, Properties env) throws RemoteException, AlreadyBoundException, Exception {
        
            if (remoteObjs.containsKey(reference)) {
                if(slowQueue){
                    //in case the queue is slow and the remote object exists, this just adds a binding to the slow queue 
                    remote.addBinding(reference, this, env, userID, slowQueue);
                    return;
                }
                else
                throw new AlreadyBoundException(reference);
        ...
        // Try to start the remtoeObject listeners
        try {
            remote.startRemoteObject(reference, this, env, userID, slowQueue);
            remoteObjs.put(reference, remote);
            // If this broker is monitored, then add a new thread -theres no
            // need to check whether the statistic thread exists or not
            if (statisticsMap != null) {
     ...
     * @param reference - Binding name
     * @throws RemoteException If the remote operation failed
     * @throws IOException If there are problems while killing the threads
     */
    ...
    public void allowRemoteAllocation(String brokerSet, String brokerName) throws Exception {
        ...
        if (remoteObjs.containsKey(brokerSet)) {
            throw new AlreadyBoundException(brokerSet);
        }
        // Try to start the remtoeObject listeners
        RemoteBrokerImpl remote = new RemoteBrokerImpl();
        try {
            remote.startRemoteBroker(brokerSet, brokerName, this, environment);
            remoteObjs.put(brokerSet, remote);
            logger.info("Object " + brokerSet + " bound");
        } catch (Exception e) {
            e.printStackTrace();
            throw new RemoteException(e);
        }
        ...
        for (RemoteObject rO : remoteObjs.values()) {
            StatisticsThread stats = new StatisticsThread(rO);
        ...
        public void allowRemoteBindingAllocation(String brokerSet, String brokerName) throws Exception {
        // Create a RemoteBrokerImpl
//        if (remoteObjs.containsKey(brokerSet)) {
//            throw new AlreadyBoundException(brokerSet);
//        }
        // Try to start the remtoeObject listeners
        SlowRemoteBrokerImpl remote = new SlowRemoteBrokerImpl();
        try {
            remote.startRemoteBroker(brokerSet, brokerName, this, environment);
            remoteObjs.put(brokerSet, remote);
            logger.info("Object " + brokerSet + " bound");
