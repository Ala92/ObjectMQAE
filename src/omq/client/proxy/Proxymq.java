...
public class Proxymq implements InvocationHandler, Remote {
     ...
     *
     * @param reference The reference represents the unique identifier of a
     * remote object
     * @param clazz It represents the real class of the remote object. With this
     * class the system can know the remoteInterface used and it can also see
     * which annotations are used
     * @param env The environment is used to know where to send the messages
     * @throws Exception
     */
    ...
    public Proxymq(String reference, Class<?> clazz, String userID, Broker broker) throws Exception {
        this.reference = reference;
        this.broker = broker;
        this.userID = userID;
        rListener = broker.getResponseListener();
        ...
        multiExchange = multi + reference;
        System.out.println("multiexchange = " + multiExchange);
        replyQueueName = env.getProperty(ParameterQueue.RPC_REPLY_QUEUE);
    ...
    public Object invoke(Object proxy, Method method, Object[] arguments) throws Throwable {
        ...
        // Publish the request
        if (request.isAsync()) {
            publishMessage(request, replyQueueName, userID);
        } else {
            response = publishSyncRequest(request, method.getReturnType(), userID);
        }
     ...
     *
     * @param request - this request contains which method and which params will
     * be invoked in the server side.
     * @param replyQueueName - this param indicates where the responseListener
     * will be listen to.
     * @throws Exception
     */
    private void publishMessage(Request request, String replyQueueName) throws Exception {
        ...
        System.out.println("Proxymq: " + reference + " invokes '" + request.getMethod() + "' , corrID: " + corrId + ", exchange: " + exchange + ", routing key: " + routingkey + ", replyQueue: "
    ...
    private void publishMessage(Request request, String replyQueueName, String userID) throws Exception {
        String corrId = request.getId();
        ...
        System.out.println("published messaged with user ID = "+ userID+ " routingkey = "+routingkey +" to exchange = "+ exchange);
        // Add the correlation ID and create a replyTo property
        BasicProperties props = new BasicProperties.Builder().appId(reference).correlationId(corrId).replyTo(replyQueueName).type(serializerType)
                .deliveryMode(deliveryMode).headers(headers).build();
     ...
     * @param request - this request contains which method and which params will
     * be invoked in the server side.
     * @param type - indicates which return type we are waiting for
    ...
    private Object publishSyncRequest(Request request, Class<?> type, String userID) throws Exception {
        String corrId = request.getId();
     ...
     * interface
     *
     * @param method - method to invoke in the server side
     ...
     * @param timeout - Timeout read in @SyncMethod.timeout(). If the timeout is
     * set in 2 seconds, the system will wait 2 seconds for the arriving of all
     * the responses.
     * @param type - Must be an Array type
     * @return resultArray
     * @throws Exception
     */
